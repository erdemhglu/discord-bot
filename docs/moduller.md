...
- `reaction_add` — yalnız **botun kendi mesajına** atılan, insan kaynaklı, sunucu içi, `GUILD_ID`/`CHANNELS` filtresinden geçen tepkiyle ilgilenir (gerisi hemen çıkar). `Reaction` olayı ne reactor'ı ne mesaj metnini taşıdığı için ikisi de `add_reaction.user`/`.message` ile HTTP'den çekilir; metni boş (embed'li) mesaja atılan tepki atlanır. `reaction_label` emoji'yi okunabilir yapar (unicode aynen, özel emoji `:ad:`, Discord'un ham `<:ad:id>` mention biçimi değil). Sonuç `remember`+`channel_note`'a, sohbet açıksa `chat.history`'ye de "(tepki 💀) \"...\" mesajına tepki verdi" satırı olarak düşer — **kendiliğinden cevap tetiklemez**, yalnız bir sonraki doğal cevaba bağlam olur. `debug` açıksa iz düşer.
