...
- `gonder(ctx, kanal, metin, ping: Option<UserId>, dosya: Option<&PathBuf>)` — mention'lar kapalı (`CreateAllowedMentions::new()`, yalnız `ping` açılır), isteğe bağlı ek dosya; başarılıysa `kendi_mesajlarim`'a (50) ekler. Kilit gönderimden SONRA alınır.
