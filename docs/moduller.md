...
- `akis_gorunum(kip, dusunce, cevap) -> Vec<String>` — kipe göre ekran: düşünme sürerken (cevap boş, düşünce var) göster kipinde "Düşünüyorum...", gizle kipinde `dusunce_sayaci` (canlı kelime sayısı); cevap başlayınca göster kipinde `tek_satir(dusunce)` hem spoiler hem `kod_bloklari` + cevap, gizle/kapalı kiplerde yalnız cevap. `tek_satir(metin)` thinking'i tek akıcı satıra indirger (her düşüncede newline atılmaz).
...
### OpenRouter (impl Bot)
- `sor_ham(Value) -> Result<String>` — POST `/chat/completions`, `choices[0].message.content`; boşsa hata. Tek HTTP noktası. Zaman aşımı: bağlantı 15 sn, iki veri arası 120 sn, toplam sınır yok (uzun düşünme kesilmez). Ağ hatası / 429 / 5xx'te (`durum_denenebilir`) 2+4 sn geri çekilip `AI_YENIDEN_DENEME` kez yeniden dener; `sor_ham_akis` de aynı mantıkta (yalnız akış açılmadan önce).
- `reasoning_kapat(govde)` — düşünme kapalıysa sağlayıcıya göre düğme: openrouter `reasoning.enabled`, mistral'e bir şey gitmez, diğerleri (qwen tarzı router) `enable_thinking:false`.
