...
- `reasoning_kapat(govde) -> bool` — düşünme kapalıysa sağlayıcıya göre düğme: openrouter `reasoning.enabled`, mistral'e bir şey gitmez, diğerleri (qwen tarzı router) `enable_thinking:false`. Alanları gerçekten eklediyse `true` döner (mandatory-reasoning yeniden denemesi için).
